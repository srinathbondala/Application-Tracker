...
async function sgetAllAccounts(userId) {
    return new Promise((resolve, reject) => {
        conn.query('SELECT * FROM accounts WHERE user_id = $1', [userId], (err, results) => {
            if (err) return reject(err);
...
async function sgetAccountById(id, userId) {
    return new Promise((resolve, reject) => {
        conn.query('SELECT * FROM accounts WHERE id = $1 AND user_id = $2', [id, userId], (err, results) => {
            if (err) return reject(err);
            if (results.rows.length === 0) return reject(new Error('Account not found'));
...
async function screateAccount(accountData, userId) {
    ...
    return new Promise((resolve, reject) => {
        ...
        conn.query(query, [account_number, account_name, account_type, balance, opening_date, last_transaction_date, status, branch_name, userId], (err, results) => {
            if (err) return reject(err);
            resolve({ id: results.rows[0].id, ...accountData });  // Return the created account
        });
    });
}
...
async function supdateAccount(id, accountData, userId) {
    ...
    return new Promise((resolve, reject) => {
        const query = `
            UPDATE accounts 
            SET account_number = $1, account_name = $2, account_type = $3, balance = $4, opening_date = $5, last_transaction_date = $6, status = $7, branch_name = $8 
            WHERE id = $9 AND user_id = $10`;
        conn.query(query, [account_number, account_name, account_type, balance, opening_date, last_transaction_date, status, branch_name, id, userId], (err, results) => {
            if (err) return reject(err);
            if (results.rowCount === 0) return reject(new Error('Account not found'));
            resolve({ id, ...accountData });    // Return the updated account
        });
    });
}
...
async function sdeleteAccount(id, userId) {
    return new Promise((resolve, reject) => {
        if (isNaN(id) || id <= 0) return reject(new Error('Invalid account ID'));
        conn.query('DELETE FROM accounts WHERE id = $1 AND user_id = $2', [id, userId], (err, results) => {
            if (err) return reject(err);
            if (results.rowCount === 0) return reject(new Error('Account not found'));
            resolve("Success");
        });
...
// Function to get accounts by branch
async function sgetAccountsByBranch(userId) {
    return new Promise((resolve, reject) => {
        const query = `
            SELECT branch_name AS branch, COUNT(*) AS accountCount
